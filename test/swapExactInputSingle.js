...
, function () {
  ...
  it("swapExactInputSingleTest: WETH -> DAI", async function () {
    ...
    await tokenInContract.connect(accounts[0]).approve(spCoinExchange.address, AMOUNT_IN);

    let beforeTokenInBalanceOf = await tokenInContract.balanceOf(accounts[0].address);
    let beforeTokenOutBalanceOf = await tokenOutContract.balanceOf(accounts[0].address);

    console.log(indent + "BEFORE TOKEN_IN  ~", TOKEN_IN_NAME, "balance:", beforeTokenInBalanceOf);
    console.log(indent + "BEFORE TOKEN_OUT ~", TOKEN_OUT_NAME, " balance:", beforeTokenOutBalanceOf);
    ...
    console.log(indent + "AFTER TOKEN_IN   ~", TOKEN_IN_NAME, "balance:", afterTokenInBalanceOf);
    console.log(indent + "AFTER TOKEN_OUT  ~", TOKEN_OUT_NAME, " balance:", afterTokenOutBalanceOf);

    console.log(indent + "DIFFERENCE       ~", TOKEN_IN_NAME, BigInt(afterTokenInBalanceOf) - BigInt(beforeTokenInBalanceOf));
    console.log(indent + "DIFFERENCE       ~", TOKEN_OUT_NAME, BigInt(afterTokenOutBalanceOf)  - BigInt(beforeTokenOutBalanceOf));
  }).timeout(100000);
